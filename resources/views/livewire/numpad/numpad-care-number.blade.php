<div @class([
    'mx-auto relative',
    "w-[$maxLength]" => $maxLength != null,
    'w-full' => is_null($maxLength),
]) x-data="{
    ...
    careCode: '{{ $careCode }}',
    get defaultValue() {
        // Format default: BYYYYMMDD (9 karakter)
        // B = Kode Rawatan (1 digit)
        const today = new Date();
        const year = today.getFullYear();
        const month = String(today.getMonth() + 1).padStart(2, '0');
        const day = String(today.getDate()).padStart(2, '0');
        return this.careCode + year + month + day;
    },
    ...
    startScan() {
        if (this.disabled) return;
        this.scanning = true;
        $dispatch('speak', { text: 'Scan barcode yang muncul pada kode booking anda' });
        clearTimeout(this.scanTimeout);
        this.scanTimeout = setTimeout(() => { this.scanning = false; }, 15000);
        $nextTick(() => {
    ...
    @if ($autoDetect)
        <div class="flex flex-col items-center justify-center gap-1 mb-4">
            <div class="text-secondary-300 text-3xl mb-3 uppercase font-bold text-center">
                Silahkan masukkan kode booking anda
            </div>
        </div>
    @endif
    ...
    <div x-show="scanning" x-transition
        ...
        <div class="text-center space-y-1">
            ...
            <p class="text-primary-700 font-semibold">Scan barcode yang muncul pada kode booking</p>
            <p class="text-gray-400 text-sm mt-2">Dekatkan barcode ke alat scan</p>
        </div>
    ...
    <div class="flex gap-2 mb-3">
        <button @click="startScan()" :disabled="disabled || scanning"
            ...
            <span x-text="scanning ? 'Menunggu scan...' : 'Scan Kode Booking'"></span>
        </button>
